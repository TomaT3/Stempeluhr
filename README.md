...
`Offline` steht nur davor, wenn die API wirklich nicht antwortet; wartet etwas
bei erreichbarer API, lautet die Zeile `2 Stempel warten auf Übertragung`.
...
Timeout 10 s, kein zweiter Versuch solange einer läuft) die Erreichbarkeit ab.
Er setzt den Banner schon beim Laden (also auch für Stempel aus einer früheren
Sitzung), löst beim Zurückkommen sofort den Nachtrag aus und verschwindet, sobald
die Queue leer ist. Ein Timeout zählt als offline — ein Server, der TCP annimmt
